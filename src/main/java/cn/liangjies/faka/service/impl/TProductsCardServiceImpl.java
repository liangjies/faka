...
public class TProductsCardServiceImpl implements TProductsCardService {
     ...
     */
    @Override
    public List <TProductsCard> buyCard(int pid, int num){
        List<TProductsCard> tProductsCards = this.tProductsCardDao.buyCard(pid, num);
        for (TProductsCard tProductsCard : tProductsCards) {
            System.out.println(tProductsCard.getId());
            this.tProductsCardDao.changeState(tProductsCard.getId());
        }

        return tProductsCards;
    }
     ...
     */
    @Override
    public List <CardType> queryCardAndType(CardSearchDto cardSearchDto){
        return this.tProductsCardDao.queryCardAndType(cardSearchDto);
    }

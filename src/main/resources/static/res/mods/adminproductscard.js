layui.define(['layer', 'table', 'form','upload'], function(exports){
	...
	form.on('select(typeid)', function(data){
		...
		$.ajax({
			url: '/Admin/products/getlistbytid',
			type: 'POST',
			dataType: 'json',
			data: {'tid': data.value},
			beforeSend: function () {
    ...
    $('#deleteALL').on('click',function () {
		...
		if(ids.length>0){
			layer.confirm('确认删除选中卡密吗？', function(index) {
				var param = {'ids': ids};
				$.ajax({
					url: '/Admin/productscard/delete',//请求的url地址
					dataType: 'json',//返回的格式为json
					data: {'id': JSON.stringify(param)},//参数值
					type: "POST"
				})
				.done(function (data) {
				if (data.code == 1) {
     ...
     $('#deleteempty').on('click',function () {
		layer.confirm('确认清空所有已删除的卡密吗？', function(index) {
			$.ajax({
				url: '/Admin/productscard/deleteempty',//请求的url地址
				dataType: 'json',//返回的格式为json
				data: {'method': "empty"},//参数值
				type: "POST"
			})
			.done(function (data) {
			if (data.code == 1) {
